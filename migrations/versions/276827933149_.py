...
def upgrade():
    ...
    statususer = sa.Enum('ACTIVE', 'INACTIVE', name='statususer')
    roleuser = sa.Enum('ADMIN', 'USER', name='roleuser')
    statususer.create(op.get_bind(), checkfirst=True)
    roleuser.create(op.get_bind(), checkfirst=True)
    ...
    with op.batch_alter_table('user', schema=None) as batch_op:
        ...
        batch_op.add_column(sa.Column('status', sa.Enum(
            'ACTIVE', 'INACTIVE', name='statususer'), nullable=False))
        batch_op.add_column(sa.Column('role', sa.Enum(
            'ADMIN', 'USER', name='roleuser'), nullable=False))
...
def downgrade():
    ...
    with op.batch_alter_table('user', schema=None) as batch_op:
        ...
        batch_op.create_unique_constraint(batch_op.f('user_email_key'), [
    ...
    statususer = sa.Enum('ACTIVE', 'INACTIVE', name='statususer')
    roleuser = sa.Enum('ADMIN', 'USER', name='roleuser')
    roleuser.drop(op.get_bind(), checkfirst=True)
    statususer.drop(op.get_bind(), checkfirst=True)
    # ### end Alembic commands ###
